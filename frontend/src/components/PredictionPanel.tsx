  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="card">
        ...
        {/* ResNet50 Deployment Message */}
        {selectedModel === 'ResNet50 (TL)' && (
          <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg animate-in fade-in">
            <div className="flex items-start">
              <svg className="w-6 h-6 text-blue-500 mr-3 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M18 5v8a2 2 0 01-2 2h-5l-5 4v-4H4a2 2 0 01-2-2V5a2 2 0 012-2h12a2 2 0 012 2z" clipRule="evenodd"></path>
              </svg>
              <div>
                <p className="font-semibold text-blue-900">Modèle non disponible en déploiement</p>
                <p className="text-sm text-blue-800 mt-1">
                  Ce modèle existe mais n'est pas accessible actuellement sur Render (limites de RAM du free tier).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 animate-in fade-in duration-500">
            {allPredictions.results.map((result: any, idx: number) => (
              <div key={idx} className={`border-2 rounded-lg p-4 hover:shadow-lg transition ${
                ...
                {result.error ? (
                  <p className="text-sm text-blue-600">Modèle non disponible en déploiement</p>
                ) : (
